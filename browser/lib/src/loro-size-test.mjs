...
console.log(
  '  Ratio:        ',
  (snap.byteLength / Buffer.byteLength(jsonAd)).toFixed(1) + 'x',
);
...
m2.set(
  ...
  'This is a longer description that contains more text to simulate real content.',
);
m2.set(
...
console.log(
  '  Ratio:        ',
  (snap2.byteLength / Buffer.byteLength(json2)).toFixed(1) + 'x',
);

// After edits
for (let i = 0; i < 10; i++)
  m2.set('https://atomicdata.dev/properties/name', 'Edit ' + (i + 1));
const snap3 = doc2.export({ mode: 'snapshot' });
console.log('\n=== After 10 name edits ===');
console.log(
  '  Loro snapshot:',
  snap3.byteLength,
  'bytes (+' + (snap3.byteLength - snap2.byteLength) + ' from history)',
);
...
console.log(
  '  Ratio:        ',
  (snap4.byteLength / Buffer.byteLength(json2)).toFixed(1) + 'x',
);
