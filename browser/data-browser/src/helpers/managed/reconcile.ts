 ...
 * identity that never got a workspace. A failed read counts as disposable, so
 * callers that only choose between the demo and a workspace are never blocked
 * by it.
 */
export async function localAgentIsDisposable(
  ...
  return (await localAgentWorkspace(store, agentSubject)) !== 'some';
}
