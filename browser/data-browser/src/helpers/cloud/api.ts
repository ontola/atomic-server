...
// fetches `${getCloudApiBase()}/<endpoint>` against the atomic-saas control
// plane (routes are `/api/me`, `/api/logout`, `/api/sync-enrollments`,
// `/api/recovery-secret`). The dev portal URL mirrors `managedServer.ts`.
// VERIFY the production base against your real deployment.

/** Base URL of the atomic-saas control-plane API (includes the `/api` prefix). */
export function getCloudApiBase(): string {
  const fromEnv =
    typeof import.meta !== 'undefined'
      ? (import.meta.env?.VITE_SAAS_API_BASE as string | undefined)
      : undefined;
