      ...
      {changes.map(subject => (
        <OldSnapshotYSyncListener
          ...
          onChange={() => {
            setOldResources(prev => ({
              ...prev,
              [subject]: proxyResource(oldResources[subject]),
            }));
          }}
        />
      ))}
...
const OldSnapshotYSyncListener: React.FC<{
  ...
  useEffect(() => {
    ...
    const unsub = store.subscribe(subject, newR => {
      ...
      // Y.Doc here: that would CRDT-merge the AI-edited doc into the pre-AI
      // snapshot and clear the diff. Collaborative Yjs updates are applied only
      // via subscribeYSync below.
    ...
    // If the resource is not a documentV2, we don't need to subscribe to YSync.
    if (!oldResource.hasClasses(dataBrowser.classes.documentV2)) {
      return unsub;
    }
    ...
    const unsubYSync = store.subscribeYSync(
      ...
      ({ docUpdate }) => {
        ...
        const yDoc = oldResource.get(dataBrowser.properties.documentContent);

        if (!isYDoc(yDoc)) {
          return;
        }

        Y.applyUpdateV2(yDoc, docUpdate);
        onChange();
      },
    );
...
/** Clears AI edit tracking when `ResourceEvents.ResourceSaved` fires (save from any UI). */
const ResourceSavedListener: React.FC<{
  ...
  useEffect(() => {
    return resource.stable.on(ResourceEvents.ResourceSaved, () => {
      onRemove(subject);
    });
  }, [resource.stable, subject, onRemove]);
