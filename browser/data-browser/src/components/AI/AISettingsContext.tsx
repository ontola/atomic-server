...
export const AISettingsContextProvider = (
  ...
  // Deliberately not defaulted to `http://localhost:11434` for the Ollama
  // preset, even though that's the address almost every install uses — it
  // stays as LocalOllamaDiscovery's / the input's suggestion instead.
  //
  ...
  //
  // Unset means no probe (`isEndpointConfigured` / the reachability check
  // below returns early on a falsy URL), so the permission prompt only ever
  // appears in response to someone deliberately configuring local AI.
