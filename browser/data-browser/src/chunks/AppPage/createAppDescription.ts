...
export const CREATE_APP_DESCRIPTION =
  ...
  "One exception: shown as a view of someone else's table, the app may edit that table's rows (its columns, and new rows of its class, never deletes) once a person allowed it. `await store.rowAccess()` says `{ status: 'granted' | 'none' | 'unavailable' }`; with `none`, show the rows read-only and offer a button that calls `await store.requestRowAccess()`, which asks the person and resolves to `{ status: 'granted' }` or `{ status: 'denied', reason }`.\n\n" +
