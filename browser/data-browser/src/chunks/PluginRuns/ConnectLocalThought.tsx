  ...
  useEffect(() => {
    ...
    platformReader(origin)
      ...
      .then(data => {
        ...
        setConstants(
          Object.fromEntries(data.parameters.map((key: string) => [key, ''])),
        );
      })
      .catch(reason => {
  ...
  const install = async () => {
    ...
    try {
      const installed = await installLocalThought(store, {
        ...
        constants,
        identity: importInstallationIdentity(connection, constants, ':api'),
        extension: 'none',
      });
...
function ImportScopeHelp() {
  return (
    <p>
      ...
      it is open. Keep this browser open to finish importing. No provider
      writes.
    </p>
  );
}
