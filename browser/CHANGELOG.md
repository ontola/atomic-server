...
- Fix: `Resource.push()` now appends to the existing Loro list (`list.push`) instead of deleting every item and writing the array back. `Resource.removeItems()` deletes matching elements by CRDT position. Two clients appending or removing different subjects on `messages`, `read`, `isA`, etc. keep both changes instead of duplicating or dropping the prefix. `set()` / `replaceListItems()` still rewrite in place. Object properties write a `LoroMap` in place instead of `JSON.stringify`. Canvas and AI chat now live-sync the resource Loro doc (`useLoroDocSync`), not only the document editor. Canvas erase deletes stroke containers by id (`removeListItemsById`) so a remote append cannot retarget an index. Resource-array drag uses `moveListItem` (delete+insert on the existing list). Unique arrays tag `resourceArrayUnique` and drop duplicate subjects after a remote import. Markdown / `description` writes a `LoroText` (prefix/suffix splice) so an AI stream append is a tail insert and concurrent edits to different ends merge. Assistant tokens persist on first chunk and `onFinish` updates that resource instead of minting a second one.
