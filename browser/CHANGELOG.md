...
- An Installation page connects the integration-proxy platforms its release manifest declares (`proxy: [...]`, #1700): per platform, "Connect <platform>" (the proxy's `/connect`, then the signed redeem) or "Use existing connection", both after a consent step, then a delegation to the Installation's app id and `integrationConnections[platform] = connection_id` on the Installation, signed by the user. Disconnect removes the delegation at the proxy and the key. Only people who can write the Installation see the controls. An Installation that declares no platform and has no recorded connection makes no request to the proxy at all, including runtime registration.
- The page registers an Installation's runtimes with the integration proxy (#1700): each node's agent, published on an `InstallationRuntime` child, becomes a runtime of the Installation's app id via `POST /runtimes {app, agent, label}`, signed v2 with the user key. It happens when an Installation page or app frame opens and when a runtime child appears, and after a delegation. It is idempotent and does not hold up the UI; failures show as a toast. Revoking or uninstalling calls `DELETE /runtimes/{agent}`. A child counts only when its genesis was signed by the agent it names. Delegations and frame capabilities now name an Installation's `integrationAppAgent` rather than the node's agent from `GET /app-agent`, which stays the fallback for `createApp` apps and older Installations.
...
- `@tomic/lib`: `signedRequestInit(url, agent, { method, body, headers })` returns the `fetch` options for a version 2 signed request, over exactly the body it sends. AtomicServer now requires version 2 on its state-changing endpoints and accepts each signature once (#1700), so the data browser and `@tomic/lib` sign every write to them with it: `/app-agent`, `/app-write`, `/plugin-view-token`, `/plugin-secret`, `/plugin-run`, `/plugin-schedule`, `/plugin-resume`, `/plugin-auto-apply`, `/plugin-trigger`, `/plugin-release*`, `/plugin-sync-*`, `/plugin-connection-*`, `/plugin-external-*`, `/integration-action*`, `/forget-peer`, `/iroh-sync`, the `/website-hosting` writes, `/app-row-grant` (grant and revoke), and the route consent answer and route-token revoke (`/plugin-route-consent`, `/plugin-route-tokens`). Reads on those paths still sign version 1.
...
- Fix: the outbox decides whether a refused commit is terminal (drop it), benign (drop it quietly) or blocking (park it) from the server's structured error code first — the `ERROR` frame's `code` over WebSocket, or the `errorCode` property on the HTTP `/commit` error body — and only falls back to matching message text when the response carries no recognized code (older servers). Previously the store's "benign drop" check (`is_genesis: true, but the resource already exists`, `Commits cannot be edited`) matched message text only, so a wording change on the server would have turned a silent bookkeeping drop into an error toast per commit, and "Commits cannot be edited" had no code at all. New `ErrorCode.IMMUTABLE_COMMIT` (10) and `isBenignTerminalCommitError(message, code)` in `@tomic/lib`.
...
- Page transition animations are now **opt-in**. They still break outside Chromium on desktop (a card-to-page morph blows up into an overlay on Firefox and on Android Chrome), so they are off until every browser we support is verified ([#1563](https://github.com/ontola/atomic-server/issues/1563)). Settings > Accessibility gains "Enable page transition animations" in place of the old "Disable" checkbox, backed by a new `viewTransitionsEnabled` localStorage key (default `false`). The old `viewTransitionsDisabled` key is no longer read: reusing it would have left transitions on for anyone who had ever toggled the old checkbox back off. The transition machinery and CSS are untouched, so turning them back on is a default flip.
