...
- Page transition animations now run only on desktop Chrome / Chromium, and stay off by default everywhere else (Firefox, Android Chrome, Safari). Those engines still leave a stuck overlay or skip the animation ([#1563](https://github.com/ontola/atomic-server/issues/1563)). The settings toggle is shown only where transitions can actually run.
