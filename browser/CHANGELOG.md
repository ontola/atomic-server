...
- The Installation page shows the connection requests of the nodes that run it (#1700, flow b): "<plugin> needs a Clockify connection", which node asked and since when, and that its runs are paused. Writers get "Connect <platform>" and "Use existing connection" on the notice (the same consent step and delegation as the Connections rows), or "Resume runs" when the platform is already connected. Recording a connection also clears that platform's open requests, in a commit signed by the user. Others see the notice without buttons. The drive's plugin list marks an Installation with an open request.
