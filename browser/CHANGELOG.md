...
- **Headless CMS:** generated Next.js and SvelteKit sites now hide forks and blog posts with a missing or future `published-at` from listings, search, routes, `sitemap.xml`, and `rss.xml`. Cmd/Ctrl+E and an **Edit this page** footer link open the current resource in the Data Browser (`/app/edit?subject=…`); pass `--cms-url` when the editor is not on the server origin. `cover-image` and `published-at` on `blogpost` are recommended rather than required. `/` is the Website resource's `homepage` property. Nav and blog cards on a language-prefixed route keep that prefix. Next.js sets `<html lang>` from the URL and re-queries AtomicServer on each request (`force-dynamic` / `cache: 'no-store'`) so listings are not baked empty at build time. Publish filtering takes `now` on an options object so `array.filter(isListedCmsResource)` does not treat the item index as a timestamp (which hid every post). Generated sites ship `/sitemap.xml`, `/robots.txt`, and `/rss.xml`. `@tomic/lib` exports `isListedCmsResource`, `cmsEditUrl`, `cmsSitemapPaths`, and the feed XML helpers. Tutorial: [Using Atomic as a headless CMS](https://docs.atomicdata.dev/headless-cms.html).
