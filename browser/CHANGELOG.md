...
- Cleanup: one `stringToSlug` in `@tomic/lib` (the data-browser copy that turns `Meat & fish` into `meat-fish`, not `meat--fish`). `useDebounce` is exported from `@tomic/react` instead of a duplicate helper. Unused `generateNonce`/`randomSubject` and deprecated e2e address-bar aliases are gone.
