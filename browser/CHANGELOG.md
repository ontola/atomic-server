...
- **Headless CMS:** generated Next.js and SvelteKit sites now hide forks and blog posts with a missing or future `published-at` from listings, search, routes, `sitemap.xml`, and `rss.xml`. On Next.js, Cmd/Ctrl+E and **Edit this page** enter in-place WYSIWYG editing via `@tomic/edit-mode` (agent secret in `localStorage`, never in the public bundle); Cmd/Ctrl+Shift+E opens the Data Browser. SvelteKit's **Edit this page** still opens `/app/edit?subject=…`. Pass `--cms-url` when the editor is not on the server origin. `cover-image` and `published-at` on `blogpost` are recommended rather than required. `/` is the Website resource's `homepage` property. Nav and blog cards on a language-prefixed route keep that prefix. Pages are prerendered (Next.js ISR) or SSR'd (SvelteKit) with `Cache-Control: public, s-maxage=60, stale-while-revalidate=86400`, and `<html lang>` is on the first HTML byte so a CDN can serve the correct locale and content without a client round-trip. Blog search filters that rendered list in the browser. Empty listings at build are retried, then expire in 60 seconds instead of sticking until a rebuild. Publish filtering takes `now` on an options object so `array.filter(isListedCmsResource)` does not treat the item index as a timestamp (which hid every post). Generated sites ship `/sitemap.xml`, `/robots.txt`, and `/rss.xml`. `@tomic/lib` exports `isListedCmsResource`, `cmsEditUrl`, `cmsSitemapPaths`, `cmsPathToSlug`, `CMS_CDN_CACHE_CONTROL`, and the feed XML helpers. Tutorial: [Using Atomic as a headless CMS](https://docs.atomicdata.dev/headless-cms.html).
