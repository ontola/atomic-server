...
- **`/search` URL shape** in two languages, bound by
  `testdata/search-query.json`: `browser/lib/src/search.ts` and
  `lib/src/client/search.rs` both build it from `SearchOpts` /
  `build_search_subject`. Tantivy (and its key-escaping requirement) is
  gone — hosted `/search` is the local-search KV engine now.
...
3. **Shared golden fixtures** for pairing URIs, `normalizeServerUrl`,
   and the `/search` URL shape — **done (2026-08-15)** (`testdata/search-query.json`,
