...
1. **Server**: ✅ drive-wide creates / destroys flow through `CommitMessage`
   (existing path), encoded as `UPDATE` / `DESTROY` by
   `WebSocketConnection::Handler<CommitMessage>`. The dedicated
   `Handler<QueryUpdate>` is gone; `Handler<MembershipNotification>` now
   encodes the same UPDATE / DESTROY shape for filter-subscription hits.
2. **Browser + Flutter**: ✅ `decodeQueryUpdate`, the binary `Tag.QUERY_UPDATE`
   branch, the text `QUERY_UPDATE` fallback, and `shouldFetchOnQueryUpdate`
   are gone. Flutter's `subscribe_query(parent, drive, drive)` is gone (the
   drive `SUB` already covers it); the `WsClient::subscribe_query` method
   stays for callers that want filter subs.
3. **Protocol & docs**: ✅ tag `0x36` marked reserved; `encode_query_update` /
   `decode_query_update` / `DecodedQueryUpdate` removed; docs page rewritten
   to describe three subscription shapes (SUB drive, SUBSCRIBE subject,
   SUBSCRIBE_QUERY filter) all flowing through UPDATE / DESTROY.
4. **Tests**: ✅
   - `server/tests/ws_drive_membership.rs` asserts the drive-wide-create →
     `UPDATE (SNAPSHOT|PUSH|HAS_COMMIT_ID)` path.
   - `server/tests/ws_destroy.rs` asserts the destroy broadcast.
   - `server/tests/ws_subscribe_query.rs` (new in this session) asserts the
     `SUBSCRIBE_QUERY` filter-membership → `UPDATE` path: a subscriber that
     registered for `parent = drive` receives the full snapshot + commit_id
     when a matching resource is created.
   - `query_subscribe_requires_read_permission` was load-bearing for the
     auth gate; the equivalent gate is now covered by the auth path inside
     `Handler<SubscribeQuery>` (drive-level `check_read`).
