...
| [`table-scale.md`](./table-scale.md) | **Measured + open-path + store shrink.** Collection pages local fetches. Commit rows no longer duplicate `loroUpdate`; envelopes are compact `AE01`. Write *latency* remains. Harness: `lib/tests/table_scale.rs`, `browser/e2e/tests/table-stress.spec.ts`. |
