...
| [`consolidation-contract.md`](./consolidation-contract.md) | **Gate:** how a consolidation PR is allowed to land — characterization tests first, then `scripts/consolidation-measure.py` so production lines in scope go down, the largest file does not grow, and behavior is pinned before the edit. |
