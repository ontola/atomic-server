...
| [`drive-as-git.md`](./drive-as-git.md) | **Exploration + prototype (2026-08-16), reversible v2.** Project a drive as a git working tree (markdown / files / JSON-AD) for backup and compatibility; export/import round-trips via `localId`, with bidirectional ProseMirror↔Markdown conversion. Snapshot export in `lib/src/git_export.rs`; not a sync transport. Complementary to the VFS and the encrypted vault. |
