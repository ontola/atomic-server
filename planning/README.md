...
| Document | Status |
| --- | --- |
| [`data-browser-ui.md`](./data-browser-ui.md) | **Phases 1-2 shipped, 3-5 proposed.** OKLCH token layer with a 104-assertion contrast gate (it caught a primary button failing AA on six of nine colour presets), and the styled-components theme facade is retired: 1752 `p.theme` reads became CSS variables, leaving `darkMode` as the theme's only member. Next: the per-property codemod for the literals that were never in the theme, then surface consolidation and chrome. |
