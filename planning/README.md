...
| Document | Status |
| --- | --- |
| [`data-browser-ui.md`](./data-browser-ui.md) | **Phase 1 shipped, Phases 2-5 proposed.** OKLCH token layer, runtime accent ramp and a theme facade over CSS variables are in, with a 104-assertion contrast gate that caught a primary button failing AA on six of nine colour presets. Next: the per-property codemod and the lint rule that keeps the drift out, then surface consolidation and chrome. |
