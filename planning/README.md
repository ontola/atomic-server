...
| [`table-scale.md`](./table-scale.md) | **Measured + open-path + store shrink.** Collection pages local fetches. Thin commit rows, compact `AE01` envelopes, commit atoms other than `subject` unindexed. Live 14 KB/row; file still COW-amplified. Harness: `lib/tests/table_scale.rs`, `browser/e2e/tests/table-stress.spec.ts`. |
