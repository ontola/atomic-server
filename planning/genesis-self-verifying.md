...
## Status & the cross-language contract

Where this stands (2026-07-10), driving toward
[`authorization-sync.md`](./authorization-sync.md) P3:

- ✅ **Server mints + verifies certs.** `commit.rs::create_did` builds the v1
  cert, derives `did:ad:<sig-over-cert>`, and stores it inline as `genesis`; the
  genesis-commit validation verifies the cert and that its signer is the commit
  signer. Dual-accept keeps legacy commit-signature DIDs working.
- ✅ **`Resource::genesis_signer()`** returns the forge-resistant creator by
  verifying the inline cert against the subject DID (the sound source P3 needs;
  not the settable `createdBy` propval).
- ✅ **Golden cross-language vectors** — `lib/src/genesis_test_vectors.json`
  (generated by `genesis::test::generate_golden_vectors`, guarded by
  `matches_the_golden_vectors`). Fixed inputs → exact cert bytes / signature /
  DID / signer DID. **This file is the contract**: the browser TS `GenesisCert`
  must reproduce every field byte-for-byte. Byte fields are hex; signature / DID
