...
- `dart/atomic_lib/rust/src/api/simple.rs` — the FRB surface (~60 functions in 7
...
- `dart/atomic_lib/lib/src/atomic_client.dart` — a static-method Dart facade that
...
`dart/atomic_lib/rust/src/api/simple.rs` mixes the generic store SDK with canvas
...
- The drive replication used for handover is the one canvas already ships in
  `sync_drive_to_server` (`dart/atomic_lib/rust/src/api/simple.rs`).
