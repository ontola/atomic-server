...
- **C16. Process-global import flags race across connections.** Confirmed. `ws_apply.rs:10,30`, `peer.rs:1186-1195`. **Fixed 2026-09-18:** see H.
- **C17. DID resources leak into other drives' watched queries.** Likely. `query_index.rs:479-482,382-445`. Read rights still apply at query time. **Fixed 2026-09-18:** see section H, third round.
- **C18. Every commit is written with `Durability::None`.** Confirmed. `redb_store.rs:477,567` (`// EXPERIMENT`). Acknowledged writes are lost on crash until the next flush. **Amended 2026-09-15:** the server and desktop run a 100ms durable-flush tick (`serve.rs`) and the WASM worker flushes on a tick, so the loss window there is 100ms. The Flutter binding opened the same redb store directly and never flushed except in `set_active_drive`, so on Android every edit since the last drive switch was lost on app kill. **Fixed 2026-09-15:** `Db::init_redb_file` owns the flush tick, so every file-backed binding gets the same 100ms bound; idle ticks no longer write. See [`atomic-lib-runtime.md`](./atomic-lib-runtime.md).
...
- **C20. Client-stamped `drive` trusted when the parent is not materialized.** Verified 2026-09-18, not exploitable as described. The stamp is only ever a rights *lookup target*, never a grant: with the parent materialized, `check_append` checks `Append` on the parent and `commit.rs` re-derives the stamp from the parent, discarding the client's; with the parent absent, `hierarchy.rs` (`check_append`, `did:` branch) accepts the stamp only if the signer has `Append` on the stamped drive *and* that drive exists here, so stamping a victim's drive is refused and stamping your own drive gains nothing (fan-out routes to your own subscribers). Residual, hygiene rather than rights: a child created while its parent was absent keeps the stamp it was created with when the parent arrives later (`parent_changed` does not fire), so the drive stamp can disagree with the parent's drive until the resource is next moved. Not fixed here.
...
- ~~`Cors::permissive()` with credentials (`serve.rs:390`); currently saved by `SameSite=Lax` on the client cookie.~~ Fixed 2026-09-18 (`server/src/cors.rs`): credentials only for the server's own origins, see H.
- `atomic_session` cookie set without `Secure` (`browser/lib/src/authentication.ts:118-121`).
- Unauthenticated WS `SUBSCRIBE_INDEX_STATUS` inserts per-connection state for arbitrary drive strings; `RBSR_FP` ranges unbounded (`web_sockets.rs:566-579,638-682`). ~~No rate limiting on `/commit`, `/upload`, `/iroh-sync`.~~ Fixed 2026-09-15 (`server/src/rate_limit.rs`, per-agent and per-peer budgets on every write endpoint and the WS `COMMIT` frame). **Amended 2026-09-18:** the HTTP `/commit` limiter keyed on the signer the body *named* before the signature was verified, so a forged commit naming a victim spent the victim's budget (targeted write lock-out, GHSA-f9rw-g3gr-3jw5, reported by hackchang). Fixed: `verify_commit_signer` runs first and the budget goes to the signer it proves; an unverifiable body spends the peer-address budget. The WS `COMMIT` frame was never affected: it is keyed on the connection's AUTH-verified agent.
- ~~Auth failures and most client errors return HTTP 500 (`helpers.rs:183`, `errors.rs:119-137`), filling Sentry;~~ Fixed 2026-09-18, see H. `default_service` logs `error!` on every unmatched request (`serve.rs:405-408`) — fixed in the second round (logs at debug).
...
- ~~`lib/src/collections.rs:257-281` `sort_resources` comparator is not a total order; reachable via `sort_by` query param, may panic on Rust ≥1.81.~~ Fixed 2026-09-18.
...
- ~~`server/src/handlers/upload.rs:47` `while let Ok(Some(field))` ends the loop silently on a multipart error and returns 200.~~ Fixed 2026-09-18.
- `lib/src/envelopes.rs:181-189`, `lib/src/sync/engine.rs:1529` `let _ =` on store writes.
- ~~`lib/src/sync/peer.rs:501,820,2079` `LIVE_PEERS.lock().unwrap()`: one panic kills live broadcast permanently.~~ Fixed 2026-09-18.
...
Fixed in the third round (`claude/technical-debt-analysis-b4ifz2-query-index`, 2026-09-18):

- **C17** (DID resources in other drives' watched queries): a first version compared a resource's `drive` stamp with the query's `drive`, but for a `did:` query `QueryFilter.drive` is `drive_prefix_from_subject` of the queried subject, which is that subject itself rather than the drive root, so legitimate rows were excluded and the `query_aggregates` tests failed. That version was reverted. The fix now carries the drive root beside the filter: when a filter is registered (`Db::register_watched_query`, and `populate_watched_queries_cache` at open) the `Db` resolves its `drive` subject's stored `drive` stamp (or the subject itself when it is a Drive) into `filter_drive_roots`, keyed by the filter's `drive` string, so the filter's identity and encoding are unchanged. `Db::filter_accepts_resource_drive` then compares a DID resource's stamp with that root on the commit path (`check_if_atom_matches_watched_query_filters`) and in the index build (`query_complex`). A resource without a stamp, or a filter whose subject is not stored here, is not excluded — a missing stamp is no evidence, and read rights still apply at query time. Regression test `did_rows_stamped_into_another_drive_stay_out_of_a_watched_query`; the `query_aggregates` tests pass.
...
- **D (CORS with credentials)**: `Cors::permissive()` stays — any origin may read, Atomic is a headless CMS — but a middleware outside it (`server/src/cors.rs`, `credentials_gate`) removes `Access-Control-Allow-Credentials` unless the `Origin` is one this server answers for: the configured domain, a subdomain of the base domain, loopback (which covers the Vite dev server on another port and `http://tauri.localhost`), or `tauri://localhost`. Same rule as B3's `host_is_served_here`. Tested against the real stack: a foreign origin gets `Allow-Origin` and no `Allow-Credentials`, an own origin gets both, preflight works for both.
- **D (client errors as 500)**: `AtomicErrorType::ParseError` maps to 400; `parse_json_ad_commit_resource` types a body that is not JSON or not a commit as a parse error; `Commit::validate_signature` and `get_agent_from_auth_values_and_check` type every failure as `Unauthorized`; the server's `get_client_agent` keeps that type instead of stringifying it. Tests post a non-JSON body (400), an unsigned commit (400), a tampered signature (401), malformed auth headers (401) and a bad auth signature (401) at real handlers.
- **C16**: `ws_apply::import_scope` is a tokio task-local carrying the peer id; `is_importing()` and `current_import_source()` read the current task's scope only. The live UPDATE import, the live `COMMIT` fallback dispatch and `apply_state_update`/`apply_destroy` run inside it, and `ingest_commit` attributes a commit to the enclosing scope's peer, so the `DbEvent` carries the peer and the push loop skips exactly that one. The push loop's global `is_importing()` check and `CommitIngestOpts::suppress_live_echo` (whose only effect was that global mute) are gone. Trade-off accepted: a peer's commit is now relayed to *other* live peers (as bulk `SYNC_PUSH` imports always were) instead of being muted for everyone during the import; the sender is skipped deterministically. Test: two interleaved imports each see only their own scope, and each stamps its own source on its `DbEvent`.
- **F**: `sort_resources` is a total order; `/upload` returns 400 on a multipart error; every `LIVE_PEERS` access goes through a poison-recovering accessor.
- **C20**: verified, see C; not exploitable via the stamp, residual noted there.
