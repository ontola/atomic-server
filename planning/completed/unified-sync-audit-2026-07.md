# Unified sync — the 2026-07 audit and consolidation record

> **Status:** Closed as-built record. Split out of
> [`../unified-sync.md`](../unified-sync.md) on 2026-09-03, which was 1081 lines
> and mostly this history.
>
> What is here: the three audit passes of 2026-07-02 (findings F1–F12), the
> dead-code / drift / consolidation inventories from the same day, the
> trust-related engineering-debt list, the Option A/B decision on manual
> `peer_sync`, and the phase checklists as they stood when they closed. The
> text is unchanged apart from relative links, which gained a `../` so they
> still resolve from `completed/`.
>
> Read it for *why* the sync code looks the way it does. Everything still open
> lives in [`../unified-sync.md`](../unified-sync.md) § Remaining work — do not
> add new work here.
...
parallel authority — see [`../unified-sync.md` § State-first wire](../unified-sync.md#state-first-wire-commit-as-provenance-envelope).
...
> Historical checklist. Unchecked lines here are carried in
> [`../unified-sync.md`](../unified-sync.md) § Remaining work — that list is the
> live one.
