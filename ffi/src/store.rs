...
impl Store {
    ...
    pub(crate) fn wrap(&self, resource: atomic_lib::Resource) -> Arc<Resource> {
        Arc::new(Resource::new(resource, self.db.clone()))
    }
}
...
impl Store {
    #[uniffi::constructor]
    pub fn open(path: String) -> Result<Arc<Self>, AtomicSdkError> {
        let base = std::path::Path::new(&path);
        let uploads = base.join("uploads");
        let db = block_on(Db::init_redb_file(base, None, &uploads)).map_err(err)?;
        Ok(Arc::new(Store { db }))
    }

    #[uniffi::constructor]
    pub fn in_memory() -> Result<Arc<Self>, AtomicSdkError> {
        let db = block_on(Db::init_redb(None)).map_err(err)?;
        Ok(Arc::new(Store { db }))
    }
    ...
    pub fn create_agent(&self, name: String) -> Result<AgentInfo, AtomicSdkError> {
        let agent = block_on(self.db.create_agent(Some(&name))).map_err(err)?;
