...
- **`dart/atomic_lib/`** — Reusable Flutter/Dart Atomic SDK (auth, sync, pairing UI, workspaces) over `atomic_lib` via `flutter_rust_bridge`. See `dart/atomic_lib/README.md` and `planning/atomic-flutter-sdk.md`.
- **`flutter/`** — Atomic Canvas app (Android/iOS/Web). Depends on `atomic_lib` for Atomic plumbing; keeps drawing/gallery UI. See `flutter/README.md` and `flutter/AGENTS.md`.
...
cargo test --manifest-path flutter/rust/Cargo.toml  # Flutter canvas app bridge (workspace-excluded, needs --manifest-path)
cargo test --manifest-path dart/atomic_lib/rust/Cargo.toml  # atomic_lib SDK bridge (workspace-excluded, needs --manifest-path)
