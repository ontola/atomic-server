...
pub fn upload_endpoint() -> Endpoint {
    Endpoint::builder("/upload")
        .params([urls::PARENT])
        .description(
            r#"In `atomic-server`, a `/upload` endpoint exists for uploading a file.
...
pub fn download_endpoint() -> Endpoint {
    Endpoint::builder("/download")
        .description(
            r#"Downloads a file referenced by a file resource.
...
- **q**: Quality setting used during encoding. Must be a number between 0 - 100."#,
        )
        .build()
}
