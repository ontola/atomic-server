...
mod path_safety_tests {
    ...
    /// Regression test for GHSA-g2q9-fjm7-cmm3: a zip entry under `assets/` carrying `..`
    /// must never write outside the drive's scoped plugin directory. Mirrors the reporter's
    /// proof of concept.
    #[test]
    fn extract_rejects_zip_slip_asset_path() {
        use std::io::Write;
        ...
        let mut zip_buf = Vec::new();
        {
            ...
            writer
                .start_file(// assets_dir is <plugins_dir>/class-extenders/scoped/<subject>/<namespace>,
                    // so five `..` escape plugins_dir entirely and land in the temp dir.
                    "assets/../../../../../atomic_zipslip_proof.txt", opts)
                .unwrap();
            writer.write_all(b"PWNED BY ZIP-SLIP POC").unwrap();
            writer.finish().unwrap();
        }
        let mut archive = ZipArchive::new(std::io::Cursor::new(zip_buf)).unwrap();

        let plugins_dir =
            std::env::temp_dir().join(format!("atomic_zipslip_extract_{}", std::process::id()));
        std::fs::create_dir_all(&plugins_dir).unwrap();
        let escaped = std::env::temp_dir().join("atomic_zipslip_proof.txt");
        ...
        assert!(result.is_err(), "traversal asset path must be rejected");
        assert!(
            !escaped.exists(),
            "file was written outside the plugins dir at {}",
            escaped.display()
        );
    ...
    async fn uninstall_rejects_path_traversal() {
        let db = Db::init_temp("uninstall_rejects_path_traversal")
            .await
            .unwrap();
        // `Db::setup` does not exist on the 0.40 line; the drive only has to be a subject
        // string here, since validation rejects the payload before any store lookup.
        let drive = "http://localhost/test-drive".to_string();
