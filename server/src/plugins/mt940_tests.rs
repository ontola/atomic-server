...
#[actix_rt::test]
async fn camt_statement_proposes_the_same_nested_transactions() {
    let keys = [
        "account",
        "currency",
        "amount",
    ...
    let properties: serde_json::Map<String, Value> = keys
        ...
        .map(|key| (format!("bank-{key}"), json!(format!("https://test/{key}"))))
        .collect();
    let input = json!({"text":include_str!("../../../integrations/mt940/fixtures/synthetic.camt053.xml"), "config":{"table":"https://test/table", "rowClass":"https://test/transaction", "properties":properties}, "trigger":{"kind":"manual","at":1}});
    let result = embedded_runtime()
        .unwrap()
        .run(
            include_str!("../../../integrations/mt940/plugin.js"),
    ...
    assert_eq!(result["intents"][0]["set"]["https://test/amount"], "-12.34");
    assert_eq!(
        result["intents"][0]["set"]["https://test/transaction-code"],
        "PMNT/ICDT/ESCT"
    );
    let invalid = input.to_string().replace("107.66", "107.67");
    let result = embedded_runtime()
