...
async fn server_tests() {
    ...
    let resp = test::call_service(&app, req.to_request()).await;
    assert!(resp.status().is_success(), "Collection pagination should work");
    let body = get_body(resp);
    assert!(body.as_str().contains("page_size"), "Should include page_size");
    assert!(body.as_str().contains("current_page"), "Should include current_page");

    // Test collection sorting
    let req = build_request_authenticated("/properties?sort_by=https://atomicdata.dev/properties/name&sort_desc=true", &appstate);
    let resp = test::call_service(&app, req.to_request()).await;
    assert!(resp.status().is_success(), "Collection sorting should work");
    let body = get_body(resp);
    assert!(body.as_str().contains("sort_by"), "Should include sort_by parameter");
    ...
    let req = build_request_authenticated("/properties?page_size=5", &appstate)
        .insert_header(("Accept", "application/ld+json"));
    let resp = test::call_service(&app, req.to_request()).await;
    assert!(resp.status().is_success(), "Collection should work with JSON-LD accept header");
    let body = get_body(resp);
    assert!(body.as_str().contains("@context"), "Should return JSON-LD format");
    assert!(
        body.as_str().contains("members") || body.as_str().contains("@members"),
