...
fn export_concurrent_with_database_operations() {
    ...
    match (json1, json2) {
        (serde_json::Value::Array(arr1), serde_json::Value::Array(arr2)) => {
            ...
            assert!(arr1.len() > 0, "First export should contain resources");
            assert!(arr2.len() > 0, "Second export should contain resources");
            ...
            assert!(
                ...
                arr1.len(), arr2.len(), diff, max_allowed_diff
            );
            ...
            let contains_properties1 = arr1.iter().any(|item| {
                item.get("@id").and_then(|id| id.as_str())
                    .map(|s| s.contains("properties")).unwrap_or(false)
            });
            ...
            assert!(contains_properties1, "First export should contain properties resources");
...
fn export_import_no_search_locks() {
    ...
    let export_handle = thread::spawn(move || {
        let mut export_cmd = assert_cmd::Command::cargo_bin("atomic-server").unwrap();
        export_cmd.args([
            "--cache-dir", &temp_cache_dir_export,
    ...
    let import_handle = thread::spawn(move || {
        ...
        let mut import_cmd = assert_cmd::Command::cargo_bin("atomic-server").unwrap();
        import_cmd.args([
            "--cache-dir", &temp_cache_dir_import,
            "--data-dir", &temp_data_dir_import,
            "import",
            "--file", &import_file_clone,
        ])
        .assert()
        .success();
    });
    ...
    export_handle.join().expect("Export should complete without locks");
    import_handle.join().expect("Import should complete without locks");
    ...
    assert!(exported_content.contains("Test Resource"), "Export should contain our test resource");
