...
mod turso_server_integration {
    use atomic_lib::{
        stores::turso::TursoConfig,
        Storelike, Resource, Value, datatype::DataType, Db,
    };
    ...
    fn test_data_compatibility() {
        ...
        let retrieved = sqlite_store.get_resource("https://example.com/server-test").unwrap();
    ...
    fn test_migration_scenarios() {
        ...
        let retrieved1 = sqlite_store.get_resource("https://example.com/migration/1").unwrap();
        let retrieved2 = sqlite_store.get_resource("https://example.com/migration/2").unwrap();
    ...
    fn test_concurrent_access_patterns() {
        ...
        for (i, subject) in resources.iter().enumerate() {
            ...
            resource.set_unsafe(
                "https://atomicdata.dev/properties/description".to_string(),
                Value::new(&format!("Concurrent test resource {}", i + 1), &DataType::String).unwrap(),
            );
            sqlite_store.add_resource(&resource).unwrap();
        }
