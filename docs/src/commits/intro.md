...
The WebSocket `COMMIT` frame is the live path (the server also accepts HTTP `POST /commit`, for scripts and other clients), and a paired peer accepts the same frame over Iroh. How commits travel between devices, and what happens to edits made offline, is the subject of [Atomic Sync](../sync.md).
