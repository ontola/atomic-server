...
- OpenID Connect is not part of this protocol, and there is no OIDC login today. The plan is for a node to optionally offer OIDC as a login: after a valid ID token it would sign a short-lived session Agent certificate, while the root key stays on the node. An OIDC token would still never sign Commits. See [#277](https://github.com/ontola/atomic-server/issues/277) and `planning/oidc-oauth.md`.
