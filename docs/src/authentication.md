...
- OpenID Connect is not part of this protocol. A node may use OIDC as the only login: after a valid ID token it signs a short-lived session Agent certificate; the root key stays on the node. The OIDC token cannot itself sign Commits. See [#277](https://github.com/ontola/atomic-server/issues/277).
