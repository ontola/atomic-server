...
- OpenID Connect is not part of this protocol. A hosted account may use OIDC (or a magic link) to resume a *session* that can unwrap a recovery envelope; that session cannot sign Commits or Authentication Resources. See [#277](https://github.com/ontola/atomic-server/issues/277).
