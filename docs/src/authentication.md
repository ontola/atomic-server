...
- OpenID Connect is not part of this protocol. A node may use OIDC as the only login: after a valid ID token it releases the Agent secret to the client. That session cannot itself sign Commits or Authentication Resources. See [#277](https://github.com/ontola/atomic-server/issues/277).
