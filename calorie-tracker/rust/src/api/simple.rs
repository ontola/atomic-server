...
pub async fn open_db(path: String) -> Result<(), String> {
    ...
    // rolled back at the next launch — the user logs a meal, the app is reaped,
    // and the meal was never there. A server amortizes the fsync over a flush
    // tick (`serve.rs`); here a commit is a user action a few times a day, so
    // paying it per commit is the right trade.
