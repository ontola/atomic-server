...
/// Sign-in / create-identity flow shared by Atomic apps.
///
/// Brand the screen with [appName], [appIcon], and [continueLabel] so each app
/// keeps its own product voice without rebuilding the secret / sync steps.
class LoginScreen extends StatefulWidget {
  final VoidCallback onLoggedIn;

  /// Product name shown on the welcome step.
  final String appName;

  /// Icon above the product name.
  final IconData appIcon;

  /// Label on the button after the secret is shown.
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Widget _buildStep(ColorScheme scheme) {
    switch (_step) {
      ...
      case _Step.welcome:
        return _buildWelcome(scheme);
      case _Step.createAgent:
        return _buildCreateAgent(scheme);
      case _Step.showSecret:
        return _buildShowSecret(scheme);
      case _Step.signIn:
        return _buildSignIn(scheme);
      case _Step.needsSync:
        return _buildNeedsSync(scheme);
    }
  }
  ...
  Widget _buildWelcome(ColorScheme scheme) {
    return Column(
      ...
      children: [
        Icon(widget.appIcon, size: 48, color: scheme.primary),
  ...
  Widget _buildCreateAgent(ColorScheme scheme) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        Text(
          'This creates a cryptographic identity and a personal workspace to store your data.',
  ...
  Widget _buildShowSecret(ColorScheme scheme) {
    return Column(
  ...
  Widget _buildSignIn(ColorScheme scheme) {
    return Column(
  ...
  Widget _buildNeedsSync(ColorScheme scheme) {
    ...
    return Column(
      ...
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        Icon(Icons.sync_problem, size: 48, color: scheme.tertiary),
        const SizedBox(height: 16),
        const Text(
          'Sync your workspace',
          style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold),
        ...
        const SizedBox(height: 12),
        Text(
          'This device does not have your workspace yet. '
        ...
        OutlinedButton.icon(
          ...
          onPressed: () async {
            ...
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                    content: Text('Workspace not synced yet — try again')),
              );
            }
          },
        ),
  ...
  Widget _errorWidget(ColorScheme scheme) {
