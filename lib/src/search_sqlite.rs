...
use crate::{
    errors::AtomicResult,
    Resource, Db, Storelike,
};

#[cfg(feature = "db")]
use fst::{IntoStreamer, Map, MapBuilder, automaton, Streamer};
#[cfg(feature = "db")]
...
impl SqliteSearchState {
    ...
    pub fn add_resource(&self, resource: &Resource, conn: &Connection) -> AtomicResult<()> {
        ...
        let description = if let Ok(crate::Value::Markdown(desc)) = resource.get(crate::urls::DESCRIPTION) {
            desc.to_string()
        } else {
            String::new()
        };
        
        let propvals_json = resource.to_json_ad()
    ...
    fn build_fst_index(&self, conn: &Connection) -> AtomicResult<()> {
        ...
        let mut stmt = conn.prepare("SELECT title, description FROM search_index")
        ...
        for row in rows {
            let (title, description) = row
                .map_err(|e| format!("Failed to get row data: {}", e))?;
        ...
        for (term, frequency) in sorted_terms {
            fst_builder.insert(&term, frequency as u64)
    ...
    pub fn fuzzy_search(&self, query: &str, max_distance: u32, limit: usize) -> AtomicResult<Vec<String>> {
        let conn = self.db.get_connection()?;
        
        // Get FST data
        let fst_data: Vec<u8> = conn.query_row(
        ...
        let fst_map = Map::new(fst_data)
            .map_err(|e| format!("Failed to load FST: {}", e))?;
        ...
        let fts_query = fuzzy_terms.iter()
    ...
    pub fn hierarchy_search(&self, parent_subject: &str, limit: usize) -> AtomicResult<Vec<String>> {
        let conn = self.db.get_connection()?;
        ...
        let rows = stmt.query_map(params![hierarchy_pattern, limit], |row| {
            Ok(row.get::<_, String>(0)?)
...
fn resource_to_hierarchy_path(resource: &Resource, _store: &Db) -> AtomicResult<String> {
    ...
    while depth < 10 { // Prevent infinite loops
        ...
        // Try to find parent
        if let Ok(parent_value) = resource.get(crate::urls::PARENT) {
            if let crate::Value::AtomicUrl(parent_url) = parent_value {
...
fn extract_terms(text: &str, terms: &mut std::collections::HashMap<String, u32>) {
    ...
    for word in text.split_whitespace() {
        let cleaned = word.trim_matches(|c: char| !c.is_alphanumeric())
            .to_lowercase();
        if cleaned.len() > 2 { // Only index terms longer than 2 characters
