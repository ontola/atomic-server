...
fn migrate_from_sled_to_sqlite(_store: &Db, _sled_path: &std::path::Path) -> AtomicResult<()> {
 ...
 mod tests {
     ...
     fn test_sled_migration_detection() {
        ...
        let result: Option<Vec<u8>> = stmt.query_row(params![b"test_subject"], |row| {
    ...
    fn test_comprehensive_data_migration() {
        ...
        // AtomicUrl
        comprehensive_propvals.insert("https://atomicdata.dev/properties/name".into(), crate::Value::String("Test Resource".into()));
        comprehensive_propvals.insert("https://atomicdata.dev/properties/parent".into(), crate::Value::AtomicUrl("https://example.com/parent".into()));
        
        // Boolean
        comprehensive_propvals.insert("https://example.com/is_active".into(), crate::Value::Boolean(true));
        comprehensive_propvals.insert("https://example.com/is_hidden".into(), crate::Value::Boolean(false));
        
        // Integer
        comprehensive_propvals.insert("https://example.com/count".into(), crate::Value::Integer(42));
        comprehensive_propvals.insert("https://example.com/negative_count".into(), crate::Value::Integer(-123));
        
        // Float
        comprehensive_propvals.insert("https://example.com/price".into(), crate::Value::Float(99.99));
        comprehensive_propvals.insert("https://example.com/ratio".into(), crate::Value::Float(-0.75));
        
        // String
        comprehensive_propvals.insert("https://atomicdata.dev/properties/description".into(), crate::Value::String("A comprehensive test resource with all data types".into()));
        
        // Markdown
        comprehensive_propvals.insert("https://example.com/content".into(), crate::Value::Markdown("# Title\n\nSome **bold** text with [links](https://example.com)".into()));
        
        // Slug
        comprehensive_propvals.insert("https://example.com/slug".into(), crate::Value::Slug("test-resource-slug".into()));
        
        // Date
        comprehensive_propvals.insert("https://example.com/created_date".into(), crate::Value::Date("2023-12-25".into()));
        
        // Timestamp
        comprehensive_propvals.insert("https://atomicdata.dev/properties/createdAt".into(), crate::Value::Timestamp(1703462400000)); // 2023-12-25 00:00:00 UTC
        
        // URI
        comprehensive_propvals.insert("https://example.com/external_link".into(), crate::Value::Uri("mailto:test@example.com".into()));
        ...
        comprehensive_propvals.insert("https://example.com/metadata".into(), crate::Value::JSON(json_data));
        ...
        comprehensive_propvals.insert("https://example.com/children".into(), crate::Value::ResourceArray(resource_array));
        
        // Unsupported value
        comprehensive_propvals.insert("https://example.com/custom_type".into(), crate::Value::Unsupported(crate::values::UnsupportedValue {
            value: "custom_data".into(),
            datatype: "https://example.com/custom_datatype".into(),
        }));
        
        test_resources.insert("https://example.com/test_resource".to_string(), comprehensive_propvals);
        
        // Create additional resources to test various edge cases
        let mut simple_propvals = PropVals::new();
        simple_propvals.insert("https://atomicdata.dev/properties/name".into(), crate::Value::String("Simple Resource".into()));
        test_resources.insert("https://example.com/simple".to_string(), simple_propvals);
        
        let mut unicode_propvals = PropVals::new();
        unicode_propvals.insert("https://atomicdata.dev/properties/name".into(), crate::Value::String("Unicode: 测试 🚀 💾 ñ".into()));
        unicode_propvals.insert("https://example.com/emoji_content".into(), crate::Value::Markdown("# Emoji Test 🎉\n\n**Bold** with 中文 and русский".into()));
        ...
        for (subject, propvals) in &test_resources {
            ...
            resources_tree.insert(subject.as_bytes(), encoded_data).unwrap();
            
            // Add some index entries for testing
            for (prop, val) in propvals {
                let index_key = format!("{}|{}|{}", prop, val.to_string(), subject);
                prop_val_sub_tree.insert(index_key.as_bytes(), b"").unwrap();
                
                let val_index_key = format!("{}|{}|{}", val.to_string(), prop, subject);
                val_prop_sub_tree.insert(val_index_key.as_bytes(), b"").unwrap();
            }
        }
        ...
        assert!(result.is_ok(), "Migration should succeed with comprehensive test data. Error: {:?}", result.err());
        ...
        for (subject, original_propvals) in &test_resources {
            ...
            let mut stmt = conn.prepare("SELECT value FROM resources WHERE key = ?1").unwrap();
            let migrated_data: Option<Vec<u8>> = stmt.query_row(params![subject.as_bytes()], |row| {
                let value: Vec<u8> = row.get(0)?;
                Ok(value)
            }).optional().unwrap();
            ...
            if let Some(data) = migrated_data {
                ...
                assert_eq!(decoded_propvals.len(), original_propvals.len(), 
                    "Property count should match for resource {}", subject);
                
                for (prop, original_val) in original_propvals {
                    assert!(decoded_propvals.contains_key(prop), 
                        "Property {} should exist in migrated data for resource {}", prop, subject);
                    ...
                    match (original_val, migrated_val) {
                        (crate::Value::JSON(original_json), crate::Value::JSON(migrated_json)) => {
                            ...
                            assert_eq!(original_str, migrated_str,
                                "JSON values should match for property {} in resource {}", prop, subject);
                        },
                        _ => {
                ...
                tracing::info!("✅ Successfully verified migration for resource: {}", subject);
            } else {
        ...
        assert!(prop_val_count > 0, "prop_val_sub index should contain migrated data");
        ...
        assert!(val_prop_count > 0, "val_prop_sub index should contain migrated data");
