...
mod tests {
    ...
    fn test_sqlite_wal_configuration() {
        ...
        let temp_store: i64 = conn.pragma_query_value(None, "temp_store", |row| row.get(0)).unwrap();
        ...
        let mmap_size: i64 = conn.pragma_query_value(None, "mmap_size", |row| row.get(0)).unwrap();
        println!("mmap_size: {}", mmap_size);
        
        let page_size: i64 = conn.pragma_query_value(None, "page_size", |row| row.get(0)).unwrap();
        println!("page_size: {}", page_size);
        
        let cache_size: i64 = conn.pragma_query_value(None, "cache_size", |row| row.get(0)).unwrap();
        println!("cache_size: {}", cache_size);
        
        let wal_autocheckpoint: i64 = conn.pragma_query_value(None, "wal_autocheckpoint", |row| row.get(0)).unwrap();
        assert_eq!(wal_autocheckpoint, 2000, "WAL autocheckpoint should be 2000");
